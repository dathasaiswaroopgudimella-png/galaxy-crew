...
export const TelemetryTerminal: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="crt-scanlines h-full w-full bg-[#05070a]/90 p-3.5 border border-white/5 rounded-md relative flex flex-col justify-between overflow-hidden">
        {/* CRT corner brackets */}
        <div className="absolute top-1.5 left-1.5 w-1 h-1 border-t border-l border-white/20 z-10" />
        <div className="absolute bottom-1.5 right-1.5 w-1 h-1 border-b border-r border-white/20 z-10" />

        <div 
          ref={containerRef}
          onScroll={handleScroll}
          className="flex-1 overflow-y-auto text-[8.5px] font-mono space-y-1 leading-normal scrollbar-thin select-text relative z-1"
          style={{ scrollbarWidth: 'thin' }}
        >
          {logs.length === 0 ? (
            <div className="text-white/20 italic font-medium uppercase tracking-wider">Awaiting telemetry telemetry stream telemetry packages...</div>
          ) : (
            logs.map((log, idx) => {
              const timeStr = new Date(log.timestamp * 1000).toLocaleTimeString([], { hour12: false });
              return (
                <div key={idx} className="flex gap-2 items-start hover:bg-white/[0.01] px-1 py-0.5 rounded transition-all">
          ...
          {logs.length > 0 && (
            <div className="flex items-center gap-1.5 text-cyan-400 font-bold mt-1">
              <span>&gt;</span>
              <span className="w-1.5 h-2 bg-cyan-400 animate-pulse" />
            </div>
          )}
